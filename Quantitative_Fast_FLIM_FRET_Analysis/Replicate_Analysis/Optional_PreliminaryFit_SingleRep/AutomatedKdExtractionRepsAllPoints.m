...
%%% THIS IS THE THIRD STEP OF THE ANALYSIS 
%%% EACH BINNED Binding curve is now analyzed and binding parameters are extracted
%%% Paramters include the Kd, lower and upper bounds of the fit confidence interval, resistance and Shape Ratio are exported

disp('[INFO] ANALYZING DATA...'); 

%%%%%%% USER DEFINED PARAMETERS %%%%%%%
%% define where binding saturation is observed 
saturation_start = 10; 
saturation_end = 50; 
%%% Path to excel file containing the Master Platemap
excel_file = 'path to platemap\MasterPlatemap.xlsx';
%%% Input sheet name for Rep1
excel_sheet ='Rep1';

%%% Path Combined Binned data. Make sure it ends with '\'
path_to_binned_csv = 'path to binned curves\';

%%% Input Cells and Transfectants with reference to the platmap

cells = {{'cell name'}}; 
transfection_references = {{'transfection'},{'transfection'},{'transfection'},{'transfection'},{'transfection'},{'transfection'}}; 
negative_references = {{'neg control'},{'neg control'},{'neg control'},{'neg control'},{'neg control'},{'neg control'}};
...
%%% Control table contains information about which positive and negative controls to consider
%%% for each experimental condition. 

control_table = cell(length(transfection_references)*length(cells)+1,9);
control_index=1;

%%% NOTE our non-transfected controls still have DMSO added to ensure experimental consistency
%%% Please ensure that your excel file has the correct cell media 
...
for c = 1:length(cells)
    ...
    if(positive_control_index > 0) && (negative_control_index > 0 ) 
        ...
        positive_filename = strcat(path_to_binned_csv,'combined_',positive_wellid,'_binned.csv') ;  
        negative_filename = strcat(path_to_binned_csv,'combined_',negative_wellid,'_binned.csv') ;  
...
%         [sat_parameter,fit_ratio] =  GetSaturationParameterAndFitRatio(sattxt,satnum,current_cell,positive_control,'DMSO'); 
        %In the case where controls are not found, a "Not Found" tag is used 
        control_table(control_index,1) = current_cell; 
        control_table(control_index,2) = {positive_control}; 
...
for c = 1:(length(transfection_references)*length(cells)+1)
   ...
   if bound_fraction_difference > 0.05
       ...
       reference_fname = strcat(path_to_binned_csv,'combined_' ,reference_well_id,'_binned.csv') ;
       ...
       [sorted_tx, Idx ] = sort(tx);
        ...
        max_data = mean(iso_ty(end-3:end));
        if(max_data <0.05)
               max_data = 0.05;
        end 

        %%% Calculate Shape Ratio (ratio)
        x = sorted_tx ; y = iso_ty; 
        y = y (x<=50); 
        x = x (x<=50); 
        x(x<0) = 0;
        y(y < 0) = 0; 
        y(y > max_data) = max_data;

        rect = max_data * max(x); %area spanned by curved 
        auc = trapz(x,y); % area under curve
        ratio = auc /(rect-auc);  % shape ratio
        control_table{c,8} = ratio; 
        
        if ratio < 2
            disp(['FitRatio is <2 for ', control_table{c,1},' + ' ,control_table{c,2}, ' curve data']);
        end
...
%Create results tables and export
saturation_results_filename = strcat(path_to_binned_csv,'SatParameters.csv');
...
for i = 2:rows 
    ...
    if(isempty(char(txt(i,3))))
        ...
        elseif(~strcmp(char(txt(i,4)),'Untransfected'))%Check that the cell is not Untransfected
            ...
            filename = strcat(path_to_binned_csv,'combined_',txt(i,1),num2str(num(i-1,1)),'_binned.csv') ;  
            ...
            if ( exist(filename, 'file'))
                ...
                num_binned_points_array(i,1) = num_of_binned_points; 
...
data_table(1,11) = {'BF_at_10-20_free_Venus'};
data_table(1,12) = {'ResistancetoTreatment'};
data_table(1,13) = {'sRatio'};
data_table(1,14) = {'Kd'};
data_table(1,15) = {'ConfInt_Lower'};
data_table(1,16) = {'ConfInt_Upper'};


results_filename = strcat(path_to_binned_csv,'PreliminaryKd_Results.xlsx'); 
