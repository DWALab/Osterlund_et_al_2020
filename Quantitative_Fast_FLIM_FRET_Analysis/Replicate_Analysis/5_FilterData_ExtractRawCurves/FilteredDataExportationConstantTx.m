...
%%% Example WellID_C21_filtered_raw.csv
    ...
    index_count = 1; 
        ...
        for u = 1:length(untransfected_indecies)
            ...
            if isnan(G) || isnan(S)
                disp('[WARNING]: Untransfected well did not have sufficient data')
                disp('It will be removed from phasor untransfected analysis');
                disp(strcat(cell_type,'>>>>',drug,'>>>>>',concentration));
                continue;
            end
