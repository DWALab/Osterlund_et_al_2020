...
path_to_donor_table4 = 'directory\' ;
...
excel_file4 = fullfile(path_to_donor_table4, 'Donor_Concentration_Table.xlsx'); 
...
[num4, txt4] = xlsread(excel_file4, 'Sheet1');
...
average_data_table(1,10) = {'Avg_Mode_Donor_Concentration'}; 
 ...
 for i = 2:rows
    ...
    if well_pos_rep4 ~= -1
        mean_donor_con(end+1) = num3(well_pos_rep4-1,6); 
        std_donor_con(end+1) = num3(well_pos_rep4-1,7); 
        median_donor_con(end+1)=num3(well_pos_rep4-1,8); 
        mode_donor_con(end+1) = num3(well_pos_rep4-1,9); 
    end
