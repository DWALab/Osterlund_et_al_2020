function [ FList ] = ParseThroughFolder(DataFolder,OutputFolder)
   ...
   %Check to make sure that the diectory does not start with '.' , '..'
    if(~(strcmpi(DirContents(i).name,'.') || strcmpi(DirContents(i).name,'..')))
        ...
        if(~DirContents(i).isdir)
            ...
            getlist=ParseThroughFolder([DataFolder,NameSeperator,DirContents(i).name], [OutputFolder,NameSeperator, DirContents(i).name]);
        end
    end
end
end